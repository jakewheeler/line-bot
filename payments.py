...
cred = credentials.Certificate(json.loads(os.getenv("FIREBASE_CREDS_JSON")))
...
def __pay_user(transaction, sender, receiver, value):
    ...
    if send_snap.get("money") < value:
        return f"You do not have enough DB to pay {rec_snap.get('name')}."
...
def __payout(line_id):
    ...
    if __user_has_been_paid_on_date(user_ref, todays_date):
        return "You have already been paid today."
...
def register(line_id, name):
    return __register_user(line_id, name)
...
def pay_user(sender_id, receiver_name, value):
    try:
        ...
        receiver_ref = db.collection("users").document(
            __get_user_by_name(receiver_name).id
        )
        return __pay_user(transaction, sender_ref, receiver_ref, value)
    except:
        return "Invalid sender or receiver."
